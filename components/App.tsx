...
const App = () => {
        ...
        const mapped: Invoice[] = data.map(f => {
          ...
          if (f.numero_fatura && f.numero_fatura.includes('/')) {
              ...
              if (!isNaN(seq)) {
                  ...
                  if (typeKey) {
                      updatedSequences[typeKey] = Math.max(updatedSequences[typeKey] || 0, seq);
                  }
              }
          }
          ...
          return {
            ...
            isCertified: !!(f as any).hash || f.status === 'Pago', 
            hash: (f as any).hash || '',
  ...
  const handleSaveInvoice = async (inv: Invoice, sId: string, action?: string) => {
      // --- REGRAS AGT: VALIDAÇÃO DE SEQUÊNCIA E CRONOLOGIA ---
      if (action === 'CERTIFY') {
          const hasUncertifiedEarlier = invoices.some(i => 
              i.seriesId === sId && 
              i.type === inv.type && 
              !i.isCertified && 
              i.id !== inv.id &&
              (new Date(i.date).getTime() < new Date(inv.date).getTime() || 
               (i.date === inv.date && i.id < inv.id))
          );

          if (hasUncertifiedEarlier) {
              alert("O documento anterior ainda não foi certificado. A certificação deve respeitar a sequência legal.");
              return;
          }
      ...
      if (action === 'CERTIFY' && !inv.isCertified) {
          ...
          // --- BAIXA DE STOCK NA CLOUD ---
          if (finalInv.type !== InvoiceType.PP && finalInv.type !== InvoiceType.OR) {
                  ...
                  if (item.productId && item.type === 'PRODUCT') {
                      try {
                          await supabase.from('movimentos_stock').insert({
                              ...
                              notas: `Faturação automática: ${finalInv.number}`,
                              empresa_id: ensureUUID(realEmpresaId || DEFAULT_FALLBACK_COMPANY_ID)
                          });
                      } catch (e) { console.error("Erro ao dar baixa de stock:", e); }
                  }
              }
          }
      } else if (!inv.isCertified) {
          finalInv.number = '---';
      }
      ...
      try {
        ...
        if (error) throw error;
          ...
          console.error("Erro ao sincronizar venda:", err.message); 
      } finally { 
          setIsLoading(false); 
      }
  ...
  const handleDeleteInvoice = async (id: string) => {
      ...
      if (inv?.isCertified) {
          alert("REGRA AGT: Documentos certificados não podem ser apagados do sistema. Utilize a função de anulação para manter o rasto fiscal.");
          return;
      }

      setInvoices(invoices.filter(i => i.id !== id));
      try {
          await supabase.from('faturas').delete().eq('id', ensureUUID(id));
      } catch (err) {
          console.error("Erro ao apagar registo da Cloud:", err);
      }
  };
  ...
  const handleSavePurchase = async (purchase: Purchase) => {
      ...
      try {
          ...
          await supabase.from('compras').upsert({
              ...
              work_location_id: ensureUUID(purchase.workLocationId)
          });
  ...
  const handleDeletePurchase = async (id: string) => {
    ...
    try {
        ...
        console.error("Erro ao apagar compra da Cloud:", err);
    }
  };
  ...
  const renderView = () => {
    ...
    switch (currentView) {
      ...
      case 'PURCHASES': return <PurchaseList purchases={purchases} onDelete={handleDeletePurchase} onCreateNew={() => setCurrentView('CREATE_PURCHASE')} onUpload={()=>{}} onSaveSupplier={s => setSuppliers([...suppliers, s])} />;
      ...
      case 'STOCK': return <StockManager products={products} setProducts={setProducts} warehouses={warehouses} setWarehouses={setWarehouses} priceTables={priceTables} setPriceTables={setPriceTables} movements={stockMovements} invoices={invoices} purchases={purchases} onStockMovement={m => setStockMovements([...stockMovements, m])} onCreateDocument={(t, i, n) => { setInvoiceInitialType(t); setInvoiceInitialData({items: i, notes: n}); setCurrentView('CREATE_INVOICE'); }} onOpenReportOverlay={() => setCurrentView('FINANCE_REPORTS')} cashRegisters={cashRegisters} clients={clients} workLocations={workLocations} series={series} />;
      case 'SETTINGS':
        return <Settings 
                  series={series} 
